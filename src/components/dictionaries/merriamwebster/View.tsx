...
import {
  MerriamWebsterResultV2,
  _MerriamWebsterSearchResult,
  parseSearchResult
} from './engine'
import { ViewPorps } from '@/components/dictionaries/helpers'

export const DictMerriamWebster: FC<ViewPorps<
  _MerriamWebsterSearchResult<string>
>> = props => {
  const [
    parsedResult,
    setParsedResult
  ] = React.useState<MerriamWebsterResultV2 | null>(null)

  React.useEffect(() => {
    let isMounted = true
    parseSearchResult(props.result).then(value => {
      if (isMounted) {
        setParsedResult(value.result)
      }
    })
